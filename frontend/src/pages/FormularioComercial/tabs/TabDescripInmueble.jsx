...
const OPTS_PISO   = ['LOSETA DE PASTA','LOSETA DE CERAMICA','CEMENTO PULIDO','FIRME DE CONCRETO']
const OPTS_MURO   = ['TABIQUE ROJO ACABADO APARENTE','CORRIDA DE YESO CON PINTURA VINILICA','APLANADO FINO CON PINTURA VINILICA']
const OPTS_PLAFON = ['CORRIDA DE YESO PINTURA VINILICA','ESTRUCTURA METALICA CON LAMINA DE ZINC']
...
function SelectMaterial({ value, opts, onChange }) {
  ...
  const handleSelect = (e) => {
    ...
    else { setModoOtro(false); onChange(e.target.value) }
  }
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'.3rem' }}>
      <select value={(esOtro||modoOtro)?'__otro__':(value||'')} onChange={handleSelect}
      ...
      {(modoOtro||esOtro) && (
        <input value={value||''} onChange={e=>onChange(e.target.value)}
          placeholder="Escribe el material…" autoFocus={modoOtro&&!esOtro}
...
const CALIDAD_PROY = [
  'EXCELENTE PARA EL TIPO PROYECTO','MUY BUENO PARA EL TIPO PROYECTO',
  'BUENO PARA EL TIPO PROYECTO','REGULAR PARA EL TIPO PROYECTO','MALO PARA EL TIPO PROYECTO',
]
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'.5rem',
        ...
        {editingLabel ? (
          <div style={{ display:'flex', gap:'.3rem', alignItems:'center' }}>
            ...
            <button onClick={()=>{ onRename(tempLabel); setEditingLabel(false) }}
              style={{ padding:'.15rem .3rem', borderRadius:'4px', background:'#c9972a', border:'none', cursor:'pointer', color:'#fff' }}>
              <Check size={11}/>
            </button>
          </div>
        ) : (
          <div style={{ display:'flex', alignItems:'center', gap:'.3rem' }}>
            ...
            <button onClick={()=>{ setTempLabel(espacio.label); setEditingLabel(true) }}
              style={{ padding:'.1rem .2rem', background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', opacity:.6 }}>
              <Pencil size={10}/>
            </button>
          </div>
        )}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'.35rem', flex:1 }}>
        ...
        <input type="text" value={espacio.valor} onChange={e=>onUpdate(e.target.value)}
...
// Botón reutilizable de mostrar/ocultar sección
function ToggleBtn({ show, onToggle, label }) {
  return (
    <button type="button" onClick={onToggle}
      style={{ display:'inline-flex', alignItems:'center', gap:'.4rem',
        padding:'.35rem .8rem', borderRadius:'7px', cursor:'pointer',
        border:`1px solid ${show?'#1e3a5f':'var(--border)'}`,
        background:show?'rgba(30,58,95,.06)':'var(--bg-card)',
        color:show?'#1e3a5f':'var(--text-muted)',
...
export default function TabDescripInmueble({ form, update }) {
  ...
  // Toggles individuales para secciones opcionales
  // Toggles sincronizados con form para que el PDF los respete
  const [mostrarCaract,     setMostrarCaract]     = useState(form.mostrarSeccion_caracts    !== false)
  const [mostrarEspacios,   setMostrarEspacios]   = useState(form.mostrarSeccion_espacios   !== false)
  const [mostrarEstructura, setMostrarEstructura] = useState(form.mostrarSeccion_estructura !== false)
  const [mostrarInst,       setMostrarInst]       = useState(form.mostrarSeccion_inst       !== false)
  const [mostrarAcabados2,  setMostrarAcabados2]  = useState(form.mostrarSeccion_acabados   === true)

  // Wrappers que guardan el estado en form para que el PDF lo lea
  const toggleCaract = () => {
    const v = !mostrarCaract; setMostrarCaract(v); update('mostrarSeccion_caracts', v)
  }
  const toggleEspacios = () => {
    const v = !mostrarEspacios; setMostrarEspacios(v); update('mostrarSeccion_espacios', v)
  }
  const toggleEstructura = () => {
    const v = !mostrarEstructura; setMostrarEstructura(v); update('mostrarSeccion_estructura', v)
  }
  const toggleInst = () => {
    const v = !mostrarInst; setMostrarInst(v); update('mostrarSeccion_inst', v)
  }
  const toggleAcabados = () => {
    const v = !mostrarAcabados2; setMostrarAcabados2(v)
    setMostrarAcabados(v); update('mostrarSeccion_acabados', v)
  }

  const espacios    = form.espaciosHabitacion?.length>0 ? form.espaciosHabitacion : ESPACIOS_DEFAULT
  const setEspacios = (nuevos) => update('espaciosHabitacion', nuevos)
  const updateEspacio = (idx,valor) => setEspacios(espacios.map((e,i)=>i===idx?{...e,valor}:e))
  const deleteEspacio = (idx)        => setEspacios(espacios.filter((_,i)=>i!==idx))
  const renameEspacio = (idx,label)  => setEspacios(espacios.map((e,i)=>i===idx?{...e,label}:e))
  const agregarEspacio = () => {
    if (!nuevoEspacio.trim()) return
    setEspacios([...espacios,{ id:`custom_${Date.now()}`, label:nuevoEspacio.trim(), valor:'' }])
    setNuevoEspacio(''); setMostrarAgregar(false)
  }

  const acabados   = form.acabados || []
  const addAcabado = () => update('acabados',[...acabados,{espacio:'',piso:'',muro:'',plafon:''}])
  const removeAcabado = (i) => update('acabados',acabados.filter((_,j)=>j!==i))
  const updAcabado    = (i,campo,valor) => update('acabados',acabados.map((a,j)=>j===i?{...a,[campo]:valor}:a))

  return (
    <div>
      {/* ── Información General (solo descripción es fija) ── */}
      <Seccion titulo="Información General del Inmueble">

        {/* Descripción — FIJA, siempre visible */}
        ...
        {/* Toggle: Características del inmueble */}
        <div style={{ marginTop:'.75rem', marginBottom:'.5rem' }}>
          <ToggleBtn show={mostrarCaract} onToggle={toggleCaract} label="características del inmueble"/>
        </div>
          ...
          <div className={styles.grid3}>
            ...
            <div className={styles.field}>
              ...
              <input className={styles.input} value={form.tiposConstruccion||form.bienQueValua||'Casa Habitación'}
                onChange={e=>update('tiposConstruccion',e.target.value)} placeholder="Casa Habitación"/>
              <span className={styles.calcNote}>Editable. Valor por defecto según el bien valuado.</span>
            </div>
            <div className={styles.field}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'.35rem' }}>
                <button type="button"
                  ...
                  style={{ width:'28px',height:'32px',borderRadius:'6px',border:'1px solid var(--border)',
                    background:'var(--bg-card)',color:'var(--text-secondary)',fontSize:'1rem',cursor:'pointer',fontWeight:700,flexShrink:0 }}>−</button>
                ...
                return (
                  <div style={{display:'flex',alignItems:'center',gap:'.5rem'}}>
                    <div className={styles.input} style={{background:'var(--bg-input)',
                      ...
                      fontWeight:800,fontSize:'.95rem',cursor:'default',display:'flex',alignItems:'center',gap:'.5rem'}}>
                      <span style={{fontVariantNumeric:'tabular-nums'}}>{rem}</span>
                      <span style={{fontSize:'.7rem',fontWeight:500,color:'var(--text-muted)'}}>= {total} − {edad}</span>
                    </div>
                  </div>
                )
              })()}
        ...
        {/* Toggle: Espacios / Habitaciones */}
        <div style={{ marginTop:'.75rem', marginBottom:'.5rem' }}>
          <ToggleBtn show={mostrarEspacios} onToggle={toggleEspacios} label="espacios/habitaciones"/>
        </div>
          ...
          <div style={{ marginTop:'.5rem' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
              ...
              <p style={{ fontWeight:700, fontSize:'.82rem', color:'var(--text-secondary)',
                textTransform:'uppercase', letterSpacing:'.04em' }}>Espacios / Habitaciones</p>
              <div style={{ display:'flex', gap:'.5rem', alignItems:'center', flexWrap:'wrap' }}>
                {!mostrarAgregar && (
                  <button type="button" onClick={()=>setMostrarAgregar(true)} className={styles.addRowBtn}>
                    <Plus size={13}/> Agregar espacio
                  </button>
                )}
                {espacios !== ESPACIOS_DEFAULT && espacios.length>0 && (
                  <button type="button" onClick={()=>setEspacios(ESPACIOS_DEFAULT)}
              ...
              <div style={{ display:'flex', gap:'.5rem', marginBottom:'.65rem',
                ...
                <input value={nuevoEspacio} onChange={e=>setNuevoEspacio(e.target.value)}
                  ...
                  style={{ flex:1, minWidth:'150px', padding:'.35rem .6rem', borderRadius:'6px',
                    border:'1px solid var(--border)', background:'var(--bg-card)',
                    color:'var(--text-primary)', fontSize:'.86rem' }} autoFocus/>
                <button type="button" onClick={agregarEspacio}
                  style={{ padding:'.35rem .8rem', borderRadius:'6px', border:'none',
                    background:'#c9972a', color:'#fff', cursor:'pointer', fontWeight:700, fontSize:'.82rem' }}>
                  ✓ Agregar
                </button>
      ...
      <div style={{ marginBottom:'.4rem', paddingLeft:'.1rem' }}>
        <ToggleBtn show={mostrarEstructura} onToggle={toggleEstructura} label="Estructura"/>
      </div>
      {mostrarEstructura && (
      ...
      {/* ── Tabla de Acabados — toggle ── */}
      <Seccion titulo="Tabla de Acabados">
        <div style={{ marginBottom:'1rem', display:'flex', alignItems:'center', gap:'1rem', flexWrap:'wrap' }}>
          <button type="button" onClick={toggleAcabados}
            style={{ display:'inline-flex', alignItems:'center', gap:'.5rem',
              padding:'.5rem 1rem', borderRadius:'8px', cursor:'pointer',
              border:`1.5px solid ${mostrarAcabados2?'#1e3a5f':'var(--border)'}`,
              background:mostrarAcabados2?'rgba(30,58,95,.08)':'var(--bg-card)',
              color:mostrarAcabados2?'#1e3a5f':'var(--text-muted)',
              fontSize:'.82rem', fontWeight:700 }}>
            {mostrarAcabados2?<EyeOff size={15}/>:<Eye size={15}/>}
            {mostrarAcabados2?'Ocultar tabla de materiales':'Mostrar tabla de materiales'}
          </button>
          {!mostrarAcabados2 && (
      ...
      {/* ── Instalaciones Especiales — opcional ── */}
      <div style={{ marginBottom:'.4rem', paddingLeft:'.1rem' }}>
        <ToggleBtn show={mostrarInst} onToggle={toggleInst} label="Instalaciones"/>
      </div>
      {mostrarInst && (
