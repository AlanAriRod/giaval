// frontend/src/pages/FormularioComercial/tabs/TabComparableTerreno.jsx — v3
...
const BASE_FACTORES = ['neg','zona','ubica','frente','sup','forma']
const BASE_LABELS   = { neg:'NEG',zona:'ZONA',ubica:'UBICA',frente:'FRENTE',sup:'SUP',forma:'FORMA' }
  ...
  return (
    <div style={{border:'1px solid var(--border)',borderRadius:'10px',background:'var(--bg-card)',overflow:'hidden',marginBottom:'.75rem'}}>
      <div onClick={()=>setOpen(o=>!o)}
        style={{display:'flex',alignItems:'center',gap:'.75rem',padding:'.65rem 1rem',background:'var(--bg-input)',borderBottom:open?'1px solid var(--border)':'none',cursor:'pointer'}}>
        ...
        <div style={{padding:'1rem'}}>
          <div className={styles.grid3} style={{marginBottom:'.85rem'}}>
            ...
            <div className={`${styles.field} ${styles.req}`}>
              <label className={`${styles.label} ${styles.req}`}>Precio de Oferta ($)</label>
            ...
            <div className={`${styles.field} ${styles.span3}`}>
              ...
              <textarea className={styles.textarea} value={comp.descripcion||comp.caracteristicas||''} onChange={e=>upd('descripcion',e.target.value)} placeholder="Portal, características del terreno…"/>
            </div>
            <div className={styles.field}>
          ...
          <div style={{display:'flex',gap:'1.5rem',flexWrap:'wrap',padding:'.55rem .8rem',background:'#0f172a',borderRadius:'7px',marginBottom:'1rem'}}>
            <div><p style={{fontSize:'.66rem',color:'#64748b',textTransform:'uppercase'}}>$/m² Base</p><p style={{fontSize:'.88rem',fontWeight:700,color:'#94a3b8'}}>{comp.oferta&&comp.supM2?fmtMXN(parseFloat(comp.oferta)/parseFloat(comp.supM2)):'—'}</p></div>
            ...
            <div><p style={{fontSize:'.66rem',color:'#64748b',textTransform:'uppercase'}}>$/m² Homologado</p><p style={{fontSize:'.88rem',fontWeight:700,color:'#c9972a'}}>{vuHom?fmtMXN(vuHom):'—'}</p></div>
          </div>
        ...
        <table style={{width:'100%',borderCollapse:'collapse',minWidth:'750px'}}>
          ...
          <tfoot>
            <tr style={{background:'#0f172a'}}>
              ...
              <td style={{border:'1px solid #1e293b',padding:'.4rem .5rem'}}>
                {enNR>0&&(
                  <button onClick={()=>update('enNRTerrenoManual',Math.round(enNR).toString())}
                    style={{padding:'.2rem .5rem',borderRadius:'5px',fontSize:'.66rem',fontWeight:700,
                      cursor:'pointer',border:'1px solid #c9972a',background:'rgba(201,151,42,.1)',
                      color:'#c9972a',whiteSpace:'nowrap'}}>
                    ↺ {Math.round(enNR).toLocaleString('es-MX')}
                  </button>
                )}
              </td>
            </tr>
...
export default function TabComparableTerreno({ form, update, updateNested, updateDeep }) {
    const [enviadoValorfisico, setEnviadoValorfisico] = useState(false)
const [mostrarBuscador,   setMostrarBuscador]   = useState(false)
  ...
  const comparables    = form.comparablesTerreno    || []
  const factoresCustom = form.factoresTerrenoCustom || []
  ...
  const cargarBD = async () => {
    setCargandoBD(true)
    try {
      const r = await authFetch('/api/comparables?tipo=terreno&limit=50')
      const d = await r.json()
      setComparablesBD(d.comparables||[])
    } catch(e){} finally{setCargandoBD(false)}
  }
  const usarComparableBD = (comp) => {
    update('comparablesTerreno',[...comparables,{
      ciudad:comp.ciudad||'', colonia:comp.colonia||'',
      oferta:String(comp.oferta||''), supM2:String(comp.superficie||''),
      descripcion:comp.descripcion||'', caracteristicas:comp.descripcion||'', informante:comp.informante||comp.fuente||'Banco BD',
  ...
  const autoLlenarConArchivo = async (e) => {
    ...
    try {
      ...
      fd.append('tipoDocumento', 'comparable_terreno')
  ...
  const agregar = () => {
    ...
    update('comparablesTerreno',[...comparables,{ciudad:'',colonia:'',telefono:'',informante:'',url:'',descripcion:'',caracteristicas:'',oferta:'',supM2:'',factores:{neg:0.95,zona:1,ubica:1,frente:1,sup:1,forma:1},fotos:[]}])
  }
  const quitar = (i) => {
    if(!window.confirm('¿Desea eliminar este comparable?\nEsta acción no se puede deshacer.')) return
    update('comparablesTerreno',comparables.filter((_,j)=>j!==i))
  }
  const actualizarCampo = (i,campo,valor) =>
    update('comparablesTerreno',comparables.map((c,j)=>j===i?{...c,[campo]:valor}:c))
  ...
  const agregarFactor = () => {
    ...
    update('factoresTerrenoCustom',[...factoresCustom,{key:`ft_${Date.now()}`,label}]); setNuevoFactor('')
  }
  const quitarFactor = (key) => update('factoresTerrenoCustom',factoresCustom.filter(f=>f.key!==key))

  return (
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.25rem',flexWrap:'wrap',gap:'.65rem'}}>
        <div>
          <h3 style={{fontWeight:800,fontSize:'1rem',color:'var(--text-primary)',marginBottom:'.2rem'}}>Comparables de Terreno — Enfoque Físico</h3>
          <p style={{fontSize:'.78rem',color:'var(--text-muted)'}}>Terrenos similares para estimar el valor del suelo.</p>
        </div>
      ...
      {comparables.length===0&&(
        <div style={{textAlign:'center',padding:'2.5rem 1rem',background:'var(--bg-input)',borderRadius:'10px',border:'2px dashed var(--border)'}}>
          <p style={{fontSize:'1.4rem',marginBottom:'.4rem'}}>🏗️</p>
          <p style={{fontWeight:700,color:'var(--text-secondary)'}}>Sin comparables de terreno</p>
      ...
      {enNRRedondeado&&(
        <div style={{marginTop:'1rem',display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem'}}>
          ...
          <div style={{padding:'1rem 1.25rem',background:'#0f172a',borderRadius:'10px'}}>
            ...
            <p style={{fontSize:'.7rem',color:'#475569',marginTop:'.2rem'}}>Se traslada automáticamente a Comp. Casa</p>
          </div>
        </div>
      )}
      {/* ── Redondear EN NR + Enviar a Conclusión ── */}
      {enNRRedondeado&&(
        <div style={{marginTop:'.75rem',display:'flex',gap:'.75rem',flexWrap:'wrap',alignItems:'center',
          ...
          <div style={{flex:1,minWidth:'200px'}}>
            <p style={{fontSize:'.7rem',fontWeight:700,color:'#1e3a5f',textTransform:'uppercase',
              letterSpacing:'.04em',marginBottom:'.2rem'}}>EN N.R. Redondeado</p>
            <p style={{fontSize:'1.1rem',fontWeight:800,color:'#1e3a5f',fontVariantNumeric:'tabular-nums'}}>
              {fmtMXN(enNRRedondeado)}
            </p>
          </div>
          <div style={{display:'flex',gap:'.5rem',flexWrap:'wrap'}}>
            <button onClick={()=>update('valorFisico',
              (Math.round(enNRRedondeado*(parseFloat(form.superficieTerreno)||areaT||0))).toFixed(2))}
      ...
      {mostrarBuscBD&&(
        <ModalBancoBD tipo="Terreno" comparables={comparablesBD} cargando={cargandoBD}
          onUsar={usarComparableBD} onCerrar={()=>setMostrarBuscBD(false)}/>
      )}

      {mostrarBuscador&&(
        <BuscadorComparables tipo="terreno" form={form} onAgregar={agregarDesdeIA} onCerrar={()=>setMostrarBuscador(false)}/>
      )}

      {/* ── Barra de Resultado Final ── */}
      {enNRRedondeado>0&&(
        <div style={{marginTop:'1.5rem',padding:'1rem 1.25rem',
          background: enviadoValorfisico?'#d1fae5':'#0f172a',
          border: enviadoValorfisico?'2px solid #16a34a':'2px solid rgba(201,151,42,.4)',
          borderRadius:'12px',transition:'all .3s',
          display:'flex',alignItems:'center',gap:'1rem',flexWrap:'wrap'}}>
          <div style={{flex:1,minWidth:'200px'}}>
            <p style={{fontSize:'.68rem',fontWeight:700,
              color: enviadoValorfisico?'#15803d':'#94a3b8',
              textTransform:'uppercase',letterSpacing:'.07em',marginBottom:'.3rem'}}>
              {enviadoValorfisico?'✓ EN N.R. enviado → Tab Costos':'EN N.R. $/m² — RESULTADO FINAL'}
            </p>
            <p style={{fontSize:'1.8rem',fontWeight:800,fontVariantNumeric:'tabular-nums',
              color: enviadoValorfisico?'#16a34a':'#c9972a',lineHeight:1}}>
            ...
            <p style={{fontSize:'.7rem',color:'#64748b',marginTop:'.2rem'}}>
              ...
              {' · '}Tab Costos lo usa para calcular el Valor Físico
            </p>
          </div>
          <button type="button"
            onClick={()=>{
              update('enNRTerrenoManual', String(Math.round(enNRRedondeado)))
              setEnviadoValorfisico(true)
              setTimeout(()=>setEnviadoValorfisico(false),3000)
            }}
            style={{padding:'.55rem 1.2rem',borderRadius:'8px',
              background: enviadoValorfisico?'#16a34a':'#c9972a',
              border:'none',color:'#fff',cursor:'pointer',
              fontSize:'.84rem',fontWeight:800,whiteSpace:'nowrap',
              boxShadow:'0 2px 8px rgba(0,0,0,.3)',transition:'all .2s'}}>
            {enviadoValorfisico?'✓ Guardado':'↺ Redondear y enviar a Costos'}
          </button>
        </div>
      )}
      {enNRRedondeado>0&&(
        <div style={{marginTop:'1.5rem',padding:'1rem 1.25rem',
          ...
          <div style={{flex:1,minWidth:'200px'}}>
            <p style={{fontSize:'.68rem',fontWeight:700,
              color: enviadoValorfisico ?'#15803d':'#94a3b8',
              textTransform:'uppercase',letterSpacing:'.07em',marginBottom:'.3rem'}}>
              {enviadoValorfisico ?'✓ Enviado a Conclusión':'VALOR FINAL — Valor Físico (EN N.R.)'}
            </p>
            <p style={{fontSize:'1.8rem',fontWeight:800,fontVariantNumeric:'tabular-nums',
              color: enviadoValorfisico ?'#16a34a':'#c9972a',lineHeight:1}}>
              {new Intl.NumberFormat('es-MX',{style:'currency',currency:'MXN'}).format(enNRRedondeado)}
          ...
          <div style={{display:'flex',gap:'.6rem',flexWrap:'wrap'}}>
            <button type="button"
              ...
              style={{padding:'.5rem 1.1rem',borderRadius:'8px',
                ...
                fontSize:'.84rem',fontWeight:800,whiteSpace:'nowrap',
                boxShadow:'0 2px 8px rgba(0,0,0,.3)',transition:'all .2s'}}>
              {enviadoValorfisico ?'✓ Enviado':'↗ Enviar a Conclusión'}
            </button>
          </div>
        </div>
      )}
